        ...
        <table id="requisitionTable" class="display responsive nowrap" style="width:100%">
            <thead>
                <tr>
                    ...
                    <th class="text-center">Purchase Req ID</th>
                    ...
                    <th class="text-center">Full Filled</th>
                    <th class="text-center">Beneficiary Reference</th>
                    <th class="text-center">My Reference</th>
                    <th class="text-center">Payment Release</th>
                    <th class="text-center">Added By Role</th>
            ...
            <tbody>
                ...
                <tr>
                    <td class="text-center">{{ $req->purchase_id }}</td>
                    <td class="text-center">{{ $req->purchase_req_id }}</td>
                    <td class="text-center">{{ $req->unique_id }}</td>
                    <td class="text-center">
                        @php
                            $status = (string)($req->status_code ?? 'Unknown');
                            $badgeClass = match($status) {
                                'Approved' => 'btn-outline-success',
                                'Pending', 'In Review', 'Unprocessed' => 'btn-outline-info',
                                'Rejected', 'Declined' => 'btn-outline-danger',
                                default => 'btn-outline-secondary'
                            };
                        @endphp
                        <button type="button" class="btn {{ $badgeClass }}">
                    ...
                    <td class="text-center">
                        @php
                            $amt = is_numeric($req->amount ?? null) ? number_format((float)$req->amount, 2) : ($req->amount ?? '');
                        @endphp
                        {{ $amt }}
                    </td>
                    ...
                    <td class="text-center">
                        @if(($req->first_line_approved ?? 0) == 1)
                    ...
                    <td class="text-center">
                        @if(($req->finanace_manager_approved ?? 0) == 1)
                            <span class="badge badge-success">Yes</span>
                        @else
                            <span class="badge badge-secondary">No</span>
                        @endif
                    ...
                    <td class="text-center">
                        @php
                            $invAmt = is_numeric($req->invoice_amount ?? null) ? number_format((float)$req->invoice_amount, 2) : ($req->invoice_amount ?? '');
                        @endphp
                        {{ $invAmt }}
                    </td>
                    <td class="text-center">{{ $req->invoice_nummber }}</td>
                    <td class="text-center">
                        @if(($req->full_filled ?? 0) == 1)
                            <span class="badge badge-success">Yes</span>
                        @else
                            <span class="badge badge-secondary">No</span>
                        @endif
                    </td>
                    <td class="text-center">{{ $req->beneficiary_reference }}</td>
                    <td class="text-center">{{ $req->my_reference }}</td>
                    <td class="text-center">{{ $req->payment_release }}</td>
                    <td class="text-center">{{ $req->added_by_role }}</td>
...
(function() {
  ...
  window.reqDT = $(tableEl).DataTable({
    ...
    lengthChange: false,
    info: true,
    order: [[0, 'desc']], // Purchase ID desc
    columnDefs: [
      { targets: -1, searchable: false },
      { targets: [0, 2, 3, 5, -2, -3], responsivePriority: 1 }
    ],
    dom: 't<"dt-bottom"ip>'
  });
